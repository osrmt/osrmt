...
import com.osrmt.apps.swingApp.artifact.form.ArtifactListExportForm;
import com.osrmt.apps.swingApp.artifact.form.ArtifactListFilterForm;
import com.osrmt.apps.swingApp.artifact.form.ArtifactListImportForm;
import com.osrmt.apps.swingApp.artifact.form.BaselineForm;
import com.osrmt.apps.swingApp.artifact.form.SystemOptionForm;
import com.osrmt.apps.swingApp.artifact.graph.TraceabilityGraph;
import com.osrmt.apps.swingApp.artifact.graph.UIGraph;
import com.osrmt.apps.swingApp.changecontrol.ChangeControlViewController;
...
import com.osrmt.apps.swingApp.system.ChangePasswordFormController;
import com.osrmt.apps.swingApp.traceability.TraceabilityViewController;
import com.osrmt.apps.swingApp.wizards.PositionWizard;
import com.osrmt.apps.swingApp.wizards.ProjectWizard;
import com.osrmt.apps.swingApp.wizards.ReferenceWizard;
import com.osrmt.apps.swingApp.wizards.ReportWizard;
import com.osrmt.apps.swingApp.wizards.SystemFormWizard;
import com.osrmt.apps.swingApp.wizards.UserAdminWizard;
import com.osrmt.apps.swingApp.wizards.UserGroupWizard;
...
public class RequirementManagerActions {
	...
	/*
	private void prepareTree(final ArtifactFormController afc, final ArtifactModel m) {
		final UITreeNode node = controller.getReqTree().getSelectedNode();
		ActionListener al = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					node.removeAllChildren();
					//TODO this must go
					RequirementServices.addChildren(node, m);
				} catch (Exception e1) {
	...
	*/
	...
	public ApplicationAction getAdminReference() {
		return new ApplicationAction(ActionGroup.REQMGRADMINREF,
				null,
	...
	public ApplicationAction getSystemReference() {
		return new ApplicationAction(ActionGroup.REQMGRSYSTEMREF,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getSystemNewArtifact() {
		return new ApplicationAction(ActionGroup.REQMGRSYSTEMNEWARTIFACT,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						final String newArtifact = JOptionPane.showInputDialog(frame,ReferenceServices.getDisplay(SystemMessageFramework.ENTERNEWARTIFACTNAME),
						...
						if (newArtifact != null) {
							...
							referenceSearch.start(ReferenceGroup.Artifact, ReferenceServices.getMsg(FormTitleFramework.COPYFIELDSSECURITYFROM), true);
							...
							referenceSearch.addOkActionListener(new UIActionListener(controller.ui) {
								public void actionExecuted(ActionEvent e) {
									...
									if (referenceSearch.getSelectedValue() != null) {
										SecurityServices.addNewArtifact(newArtifact, referenceSearch.getSelectedValue().getRefId());
	...
	public ApplicationAction getSystemNewArtifactField() {
		return new ApplicationAction(ActionGroup.REQMGRSYSTEMNEWARTIFACTFIELD,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						final UIReferenceSearch referenceSearch = new UIReferenceSearch(controller.ui);
						referenceSearch.start(ReferenceGroup.Artifact, ReferenceServices.getMsg(FormTitleFramework.SELECTARTIFACTFORMTOEDIT), true);
						referenceSearch.setSize(UIProperties.getDIALOG_SIZE_450_330());
						referenceSearch.addOkActionListener(new UIActionListener(controller.ui) {
							public void actionExecuted(ActionEvent e) {
								try {
	...
	private void addNewField(final ReferenceDisplay artifact) throws Exception {
		...
		final UIReferenceSearch referenceSearch = new UIReferenceSearch(controller.ui);
		referenceSearch.start(ReferenceGroup.ControlTemplate, ReferenceServices.getMsg(FormTitleFramework.SELECTFIELDTEMPLATE), false);
		referenceSearch.setSize(UIProperties.getDIALOG_SIZE_450_330());
		referenceSearch.addOkActionListener(new UIActionListener(controller.ui) {
			public void actionExecuted(ActionEvent e) {
				try {
				...
				if (referenceSearch.getSelectedValues() != null) {
					...
					referenceSearch.dispose();
					String msg = referenceSearch.getSelectedValues().size() == 1 ? referenceSearch.getSelectedValues().getFirst().getDisplay()
	...
	public ApplicationAction getSystemImport() {
		return new ApplicationAction(ActionGroup.REQMGRSYSTEMIMPORT,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getSystemDebug() {
		return new ApplicationAction(ActionGroup.REQMGRSYSTEMDEBUG,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getSystemBaseline() {
		return new ApplicationAction(ActionGroup.REQMGRSYSTEMBASELINE,
	...
	public ApplicationAction getSystemOptions() {
		return new ApplicationAction(ActionGroup.REQMGRSYSTEMOPTIONS,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getAdminUsers() {
		return new ApplicationAction(ActionGroup.REQMGRADMINUSERS,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getAdminPositions() {
		return new ApplicationAction(ActionGroup.REQMGRADMINPOSITIONS,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getAdminUserGroups() {
		return new ApplicationAction(ActionGroup.REQMGRADMINUSERGROUPS,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getAdminProjects() {
		return new ApplicationAction(ActionGroup.REQMGRADMINPROJECTS,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getSystemForms() {
		return new ApplicationAction(ActionGroup.REQMGRSYSTEMFORMS,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getToolsReports() {
		return new ApplicationAction(ActionGroup.REQMGRTOOLSREPORTS,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getToolsTrace() {
		return new ApplicationAction(ActionGroup.REQMGRTOOLSTRACE,
			getProductOpenControlState(),
		new UIActionListener(controller.ui) {
			public void actionExecuted(ActionEvent me) {
	...
	public ApplicationAction getToolsTraceBranch() {
		return new ApplicationAction(ActionGroup.REQMGRTOOLSTRACEBRANCH,
			getProductOpenControlState(),
		new UIActionListener(controller.ui) {
			public void actionExecuted(ActionEvent me) {
				try {
					if (controller.getReqTree().getSelectedNode() != null) {
						ArtifactModel am = (ArtifactModel) controller.getReqTree().getSelectedNode().getUserObject();
	...
	public ApplicationAction getToolsImpact() {
		return new ApplicationAction(ActionGroup.REQMGRTOOLSIMPACT,
				getTreeArtifactSelected(), 
		new UIActionListener(controller.ui) {
			public void actionExecuted(ActionEvent me) {
	...
	public ApplicationAction getToolsFilter() {
		return new ApplicationAction(ActionGroup.REQMGRTOOLSFILTER,
				new ControlState() {
		...
		new UIActionListener(controller.ui) {
			public void actionExecuted(ActionEvent me) {
				final ArtifactModel a = (ArtifactModel) controller.getReqTree().getSelectedNode().getUserObject();
				final ArtifactListFilterForm form = new ArtifactListFilterForm(frame, a.getArtifactRefId(), controller.getFilterParameters(a.getArtifactRefId()));
	...
	public ApplicationAction getEditArtifact() {
		return new ApplicationAction(ActionGroup.REQMGREDITARTIFACT,
			getArtifactListSelected(),
					...
					if (selected != null && selected instanceof ArtifactModel) {
						...
						SwingWorker sw = new SwingWorker(frame) {
							...
							protected void doNonUILogic() throws Exception {
								ArtifactModel am = RequirementServices.getArtifact(selectedArtifact.getArtifactId());
								if (SecurityServices.isReadOnlyControlList(ApplicationFramework.get(ApplicationGroup.ARTIFACTFORM), am.getArtifactRefId())) {
									ArtifactReadOnlyFormController afc = new ArtifactReadOnlyFormController(frame);
									afc.start(am);
								} else {
									...
									afc.start(am.getArtifactRefId(), am, selectedArtifact, (UITreeNode) controller.getReqTree().getSelectedNode());
								}
							}
	...
	private ControlState canArtifactMoveUp() {
		return 	new ControlState() {
			public boolean getEnabled() {
				if (isTreeArtifactModelSelected()
				&& controller.getReqTree().getSelectedNodes().length ==1) {
	...
	private void moveNode(UITreeNode node, UITreeNode parent, boolean up, int prevIndex, int newIndex) {
		...
		controller.getReqTree().getTree().setSelectionPath(new TreePath(((UITreeNode)parent.getChildAt(newIndex)).getPath()));
	...
	public ApplicationAction getEditMoveUp() {
		return new ApplicationAction(ActionGroup.REQMGRMOVEUP,
				canArtifactMoveUp(),
		new DoubleClickListener() {
			public void call(ActionEvent me) {
				try {
	...
	public ApplicationAction getEditMoveDown() {
		return new ApplicationAction(ActionGroup.REQMGRMOVEDOWN,
				canArtifactMoveDown(),
		new DoubleClickListener() {
			public void call(ActionEvent me) {
				try {
	...
	public ApplicationAction getEditDelete() {
		return new ApplicationAction(ActionGroup.REQMGREDITDELETE,
				getTreeArtifactSelected(), 
		new UIActionListener(controller.ui) {
			public void actionExecuted(ActionEvent ae) {
				try {
					if (controller.getReqTree().isNodeSelected()) {
						UITreeNode[] selectedNodes = new UITreeNode[controller.getReqTree().getSelectedNodes().length];
...
//							ArtifactList children = RequirementServices.getAllSameChildren(a.getArtifactRefId(), a.getArtifactId());
	//						deleteList.addAll(children);
		//					deletedParentNodes.add(parentNode);
						...
						if (JOptionPane.YES_OPTION != JOptionPane.showConfirmDialog(frame, ReferenceServices.getDisplay(FormButtonTextGroup.DELETE) + " " 
								+ recursiveDeleteCount + " " +  ReferenceServices.getDisplay(SystemMessageGroup.ARTIFACTSFOUND))) {
							return;
						}
						/*		
						Enumeration e1 = deleteList.elements();
	...
	public ApplicationAction getEditFind() {
		return new ApplicationAction(ActionGroup.REQMGREDITFIND,
				getTreeArtifactSelected(), 
		new UIActionListener(controller.ui) {
			public void actionExecuted(ActionEvent ae) {
				try {
					if (controller.getReqTree().isNodeSelected()) {
						final ArtifactModel a = (ArtifactModel) controller.getReqTree().getSelectedNode().getUserObject();
						String searchString = JOptionPane.showInputDialog(frame,ReferenceServices.getMsg(FormButtonTextFramework.SEARCHANYPARTOFARTIFACTS) + ": " + ReferenceServices.getDisplay(a.getArtifactRefId()),
								ReferenceServices.getMsg(FormTitleFramework.SEARCH) + " " + ReferenceServices.getDisplay(a.getArtifactRefId()),JOptionPane.PLAIN_MESSAGE);
	...
	public ApplicationAction getFileNewProduct() {
		return   new ApplicationAction(ActionGroup.REQMGRNEWPRODUCT,
				null,
				new UIActionListener(controller.ui) {
		public void actionExecuted(ActionEvent ae) throws Exception {
			inputNewProduct = new UIInputBox(
			new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					try {
			...
			},ReferenceServices.getMsg(FormTitleFramework.NEWPRODUCT),controller.ui,ReferenceServices.getMsg(FormButtonTextFramework.PRODUCT));
	...
	public ApplicationAction getFileOpenProduct() {
		return   new ApplicationAction(ActionGroup.REQMGRFILEOPEN,
				null,
				new UIActionListener(controller.ui) {
		public void actionExecuted(ActionEvent ae) throws Exception {
			...
			referenceSearch.addOkActionListener(new UIActionListener(controller.ui) {
				public void actionExecuted(ActionEvent e) {
					...
					//TODO should force selection before ok
					if (referenceSearch.getSelectedValue() != null) {
						ao.setProductRefId(referenceSearch.getSelectedValue().getRefId(), ReferenceServices.getDisplay(referenceSearch.getSelectedValue().getRefId()));
						Application.setObject(ao);
						controller.ui.setTitle(ao.getProductRefDisplay() + " - " + Application.getUser().getUserLogin() 
								+ "@" + SecurityServices.getEnvironment());
						referenceSearch.dispose();
	...
	public ApplicationAction getFileCloseProduct() {
		return new ApplicationAction(ActionGroup.REQMGRFILECLOSE,
		getProductOpenControlState(),
		new UIActionListener(controller.ui) {
			public void actionExecuted(ActionEvent ae) throws Exception {
	...
	public ApplicationAction getFileNewArtifact() {
		return new ApplicationAction(ActionGroup.REQMGRFILENEW,
				getProductOpenControlState(),
		new UIActionListener(controller.ui) {
		public void actionExecuted(ActionEvent ae) {
			if (ae.getSource() != null && ae.getSource() instanceof UIMenuItem) {
				...
				if (controller.getReqTree().isNodeSelected()) {
					...
					if (o != null && o instanceof ArtifactModel) {
						ArtifactModel parentArtifact = (ArtifactModel) controller.getReqTree().getSelectedObject();
						ArtifactFormController afc = new ArtifactFormController(frame);
						//prepareTree(afc, parentArtifact);
						afc.startChild(avm.getAppTypeRefId(), parentArtifact, (UITreeNode) controller.getReqTree().getSelectedNode());
					}
				} else {
					ArtifactFormController afc = new ArtifactFormController(frame);
				...
				if (isTreeArtifactSelected()) {
					...
					ArtifactFormController afc = new ArtifactFormController(frame);
					//prepareTree(afc, parentArtifact);
					afc.startChild(parentArtifact.getArtifactRefId(), parentArtifact, (UITreeNode) controller.getReqTree().getSelectedNode());
					resetSystemState();
				} else {
					Debug.displayGUIMessage(ReferenceServices.getMsg(SystemMessageFramework.SELECTANARTIFACT));
				}
			}
	...
	public ApplicationAction getFileRefresh() {
		return new ApplicationAction(ActionGroup.REQMGRFILEREFRESH,
				getProductOpenControlState(),
				 new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent ae) throws Exception {
						controller.buildTree();
	...
	public ApplicationAction getFileImport() {
		return new ApplicationAction(ActionGroup.REQMGRFILEIMPORT,
				getProductOpenControlState(),
				 new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent ae) throws Exception {
						ArtifactModel parentArtifact = new ArtifactModel();
						if (controller.getReqTree().getSelectedNode() != null && controller.getReqTree().getSelectedNode().getUserObject() instanceof ArtifactModel) {
							final ArtifactModel a = (ArtifactModel) controller.getReqTree().getSelectedNode().getUserObject();
							parentArtifact = a;
						}
	...
	public ApplicationAction getFileExport() {
		return new ApplicationAction(ActionGroup.REQMGRFILEEXPORT,
				getProductOpenControlState(),
				 new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent ae) throws Exception {
						ArtifactListExportForm form = new ArtifactListExportForm(frame, controller.getCurrentArtifactList());
						form.setVisible(true);
					}
				});
	}
	...
	public ApplicationAction getExportBranch() {
		return new ApplicationAction(ActionGroup.REQMGREXPORTBRANCH,
				getTreeArtifactSelected(),
				 new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent ae) throws Exception {
						if (controller.getReqTree().getSelectedNode() != null) {
							...
							if (am.getArtifactId() > 0) {
								list = RequirementServices.getAllSameChildren(am.getArtifactRefId(), am.getArtifactId());
	...
	public ApplicationAction getFilePrint() {
		return new ApplicationAction(ActionGroup.REQMGRFILEPRINT,
				getArtifactListSelected(),
				new UIActionListener(controller.ui) {
			public void actionExecuted(ActionEvent me) {
				try {
	...
	public ApplicationAction getFileChangePassword() {
		return new ApplicationAction(ActionGroup.REQMGRCHANGEPASSWORD,
				null,
				 new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent ae) throws Exception {
	...
	public ApplicationAction getFileExit() {
		return new ApplicationAction(ActionGroup.REQMGRFILEEXIT,
				null,
				 new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent ae) {
						try {
	...
	public ApplicationAction getSystemLog() {
		return new ApplicationAction(ActionGroup.REQMGRSYSTEMLOG,
				null,
				 new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent ae) throws Exception {
	...
	public ApplicationAction getHelpAbout() {
		checkVersion();
		return new ApplicationAction(ActionGroup.REQMGRHELPABOUT,
				null,
				 new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent ae) throws Exception {
	...
	public ApplicationAction getTraceabilityView() {
		return new ApplicationAction(ActionGroup.VIEWTRACEABILITY,
				getProductOpenControlState(),
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	public ApplicationAction getChangeControlView() {
		return new ApplicationAction(ActionGroup.VIEWCHANGECONTROL,
				null,
				new UIActionListener(controller.ui) {
					public void actionExecuted(ActionEvent me) {
						try {
	...
	private void checkVersion() {
		try {
			...
			if (!clientdate.equals(serverdate)) {
				Debug.LogError(this, ReferenceServices.getMsg(SystemMessageFramework.CLIENT) + " last updated " + clientdate + " != "+ ReferenceServices.getMsg(SystemMessageFramework.SERVER) +  " last updated " + serverdate);
			}
			if (!clientdate.equals(dbdate)) {
				Debug.LogError(this, ReferenceServices.getMsg(SystemMessageFramework.CLIENT) + " last updated " + clientdate + " != " +ReferenceServices.getMsg(SystemMessageFramework.DATABASE) +  " last updated " + dbdate);
			}
			if (!serverdate.equals(dbdate)) {
				Debug.LogError(this, ReferenceServices.getMsg(SystemMessageFramework.SERVER) + " last updated " + serverdate + " != " + ReferenceServices.getMsg(SystemMessageFramework.DATABASE) +  " last updated " + dbdate);
			}
		} catch (Exception ex) {
			Debug.LogException(this, ex);
		}
		...
		return 	new ControlState() {
			public boolean getEnabled() {
				return controller.getTableList().isRowSelected()
						&& ApplicationObject.getApplicationProductRefId()>0;
			}
		};
	}
	...
	public void showArtifactPopupMenu(int x, int y, ArtifactModel model, UITreeNode node, ApplicationActionList actions) {
	...
	private void addRefresh(final int productRefId, UIPopupMenu pop, ApplicationActionList actions, MenuManager menuManager, final UITreeNode node) {
		...
		menuManager.addMenuItem("Menu File - Refresh", ReferenceServices.getMsg(FormButtonTextFramework.POPUPREFRESH), action, false, null, pop);
		
	}
	
	private void addDelete(UIPopupMenu pop, ApplicationActionList actions, MenuManager menuManager, final UITreeNode node) {
		ApplicationAction action = actions.getAction(ActionGroup.REQMGREDITDELETE);
		menuManager.addMenuItem("Menu Edit - Delete", ReferenceServices.getMsg(FormButtonTextFramework.DELETE), action, false, null, pop);
		
	}
	
	private void addEditArtifact(UIPopupMenu pop, ApplicationActionList actions, MenuManager menuManager, final UITreeNode node) {
		ApplicationAction action = actions.getAction(ActionGroup.REQMGREDITARTIFACT);
		menuManager.addMenuItem("Menu Edit - Artifact", ReferenceServices.getMsg(FormButtonTextFramework.EDIT), action, false, null, pop);
		
	}
	
	private void addTraceArtifacts(UIPopupMenu pop, ApplicationActionList actions, MenuManager menuManager, final UITreeNode node) {
		ApplicationAction action = actions.getAction(ActionGroup.REQMGRTOOLSTRACEBRANCH);
		menuManager.addMenuItem("Menu Tools - Trace", ReferenceServices.getMsg(FormButtonTextFramework.TRACEBRANCH), action, false, null, pop);
	...
	private void addExport(UIPopupMenu pop, ApplicationActionList actions, MenuManager menuManager, final UITreeNode node) {
		ApplicationAction action = actions.getAction(ActionGroup.REQMGREXPORTBRANCH);
		menuManager.addMenuItem("Menu File - Export", ReferenceServices.getMsg(SystemMessageFramework.EXPORTARTIFACTS), action, false, null, pop);
